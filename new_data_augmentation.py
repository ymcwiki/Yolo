...
def parse_args():
    ...
    parser.add_argument('--train_dir', type=str, default='D:/Temp/Yolov8/defect_detection/训练用照片', help='训练用照片目录')
...
def parse_xml(xml_file, defect_type=None):
    """解析XML标注文件"""
    try:
        # 使用二进制模式打开并解析XML文件
        ...
        for obj in root.findall('object'):
            ...
            # 如果XML中没有指定缺陷类型但传入了defect_type参数，使用传入的类型
            if defect_type and name == 'defect':
                name = defect_type
...
def create_yolo_txt(objects, width, height, class_map):
    """创建YOLO格式的txt标注文件"""
    lines = []
    for obj in objects:
        # 计算归一化的中心点坐标和宽高
        ...
        # 获取类别ID
        class_name = obj['name']
        class_id = class_map.get(class_name, 0)  # 如果类别不在映射中，默认为0
...
def process_directory(folder_path, defect_type, augmentations, aug_count):
    """处理指定目录中的图像"""
    images_and_annotations = []
    
    # 获取文件夹标识符(只在此处定义一次，用于所有图像)
    folder_name = os.path.basename(folder_path)
    folder_short_id = "".join([c for c in folder_name if c.isalnum()])[:5]
    ...
    for file in os.listdir(folder_path):
        ...
        if any(file_lower.endswith(ext) for ext in img_extensions):
            img_files.append(os.path.join(folder_path, file))
    
    print(f"在 {folder_path} 中找到 {len(img_files)} 张图像")
    ...
    for img_path in img_files:
        ...
        # 寻找对应的XML标注文件
        xml_file = os.path.join(folder_path, f"{img_stem}.xml")
        if not os.path.exists(xml_file):
            print(f"警告: 未找到 {img_file} 的XML标注文件")
            continue
        
        # 使用numpy和文件IO读取图像，避免OpenCV中文路径问题
        try:
            ...
            print(f"警告: 读取图像 {img_file} 时出错: {e}")
            continue
        
        # 解析XML标注
        try:
            annotation = parse_xml(xml_file, defect_type)
            
            # 如果是正常样本但没有标注对象，添加一个"normal"类型的伪标注
            if defect_type == "normal" and len(annotation['objects']) == 0:
                # 创建一个覆盖整个图像的标注框
                annotation['objects'].append({
                    'name': 'normal',
                    'bbox': [0, 0, annotation['width']-1, annotation['height']-1]
                })
        ...
        # 创建新的文件名（添加文件夹标识前缀）
        new_img_file = f"{folder_short_id}_{img_file}"
        
        # 添加原始图像
        images_and_annotations.append((new_img_file, img, annotation))
        ...
        for i in range(aug_count):
            ...
            # 生成增强后的文件名
            aug_filename = f"{folder_short_id}_{img_stem}_aug_{i}{img_ext}"
...
def save_to_dataset(images_and_annotations, output_dir, subset, class_map):
    """保存增强后的图像和标注到数据集目录"""
    ...
    for filename, img, anno in images_and_annotations:
        ...
        if ret:
            ...
            print(f"警告: 无法编码图像 {filename}")
            continue
        
        # 保存YOLO格式标注
        yolo_content = create_yolo_txt(anno['objects'], anno['width'], anno['height'], class_map)
...
def detect_class_names_from_xml(train_dir):
    """从XML文件中检测所有类别名称"""
    class_names = set()
...
def main():
    ...
    # 检测训练目录下的所有子目录
    subdirs = [d for d in os.listdir(args.train_dir) if os.path.isdir(os.path.join(args.train_dir, d))]
    
    # 筛选出有效的子目录（包含图像和XML文件）
    valid_subdirs = []
    for subdir in subdirs:
        dir_path = os.path.join(args.train_dir, subdir)
        
        # 使用os.listdir避免中文路径问题
        img_files = []
        xml_files = []
        
        for file in os.listdir(dir_path):
            file_lower = file.lower()
            if file_lower.endswith('.jpg') or file_lower.endswith('.jpeg') or file_lower.endswith('.png'):
    ...
    print(f"找到 {len(valid_subdirs)} 个有效子目录：")
    for i, subdir in enumerate(valid_subdirs, 1):
        print(f"{i}. {subdir}")
    
    # 检测所有类别
    class_names = detect_class_names_from_xml(args.train_dir)
    print(f"\n从XML文件中检测到以下类别：{', '.join(class_names)}")
    ...
    for subdir in valid_subdirs:
        ...
        # 根据目录名推断缺陷类型
        if '合格' in subdir or '正常' in subdir:
            defect_type = 'normal'
        else:
            # 从文件夹名称中提取缺陷类型
            match = re.search(r'不合格照片-([^-]+)', subdir)
            if match:
                defect_type = match.group(1)
            else:
                match = re.search(r'不合格照片\s*-\s*([^-]+)', subdir)
                if match:
                    defect_type = match.group(1)
                else:
                    # 检查其他可能的模式
        ...
        print(f"\n正在处理 {subdir} (缺陷类型: {defect_type})...")
        images, orig_count = process_directory(dir_path, defect_type, selected_methods, aug_per_image)
        all_images.extend(images)
        total_orig_count += orig_count
    ...
    save_to_dataset(train_images, args.output_dir, 'train', class_map)
    
    print(f"正在保存验证集 ({len(val_images)} 张图像)...")
    save_to_dataset(val_images, args.output_dir, 'val', class_map)
    
    print(f"正在保存测试集 ({len(test_images)} 张图像)...")
    save_to_dataset(test_images, args.output_dir, 'test', class_map)
    
    # 创建data.yaml文件
    create_data_yaml(args.output_dir, class_names)
    
    print("\n数据增强完成!")
    print(f"总计生成 {n_images} 张图像 (原始: {total_orig_count}, 增强: {n_images - total_orig_count})")
